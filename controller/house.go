...
import (
	...
	getArea "ihomeMs/proto/getArea/proto/getArea"
	getImg "ihomeMs/proto/getImg/proto/getImg"
	houseMicro "ihomeMs/proto/house/proto/house"
	orderMicro "ihomeMs/proto/order/proto/userOrder"
	register "ihomeMs/proto/register/proto/register"
	user "ihomeMs/proto/user/proto/user"
	"ihomeMs/utils"
	"image/png"
	"net/http"
	"path"
...
func PostAvatar(ctx *gin.Context) {
	...
	if fileExt != ".png" && fileExt != ".jpg" && fileExt != ".jpeg" && fileExt != ".JPEG" && fileExt != ".PNG" && fileExt != ".JPG" {
		fmt.Println("文件类型错误,请重新选择", fileExt)
		return
	}
	//只读的文件指针
...
func GetUserHouses(ctx *gin.Context) {
	...
	//调用远程服务
	resp, err := microClient.GetHouseInfo(context.TODO(), &houseMicro.GetReq{UserName: userName.(string)}, opts)

	if err != nil {
		fmt.Println("调用微服务go.micro.srv.house失败", err)
	}
	fmt.Println(resp)
	//返回数据
	ctx.JSON(http.StatusOK, resp)
...
func PutComment(ctx *gin.Context) {
	...
	err := ctx.Bind(&commentData)
	if err != nil {
		fmt.Println("绑定评价内容失败...", err)
	}
	//通过micro客户端调用远程服务
	microClient := orderMicro.NewUserOrderService("go.micro.srv.userOrder", utils.GetMicroClient())
	resp, _ := microClient.UpdateOrderComment(context.TODO(), &orderMicro.OrderCommentReq{
